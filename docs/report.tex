...
for(i = 0; i < relations.length; i=i+2){
	...
	These features would be fairly use-case dependent and since they would have an effect on the cost of running a workflow, they should only be implemented in instances where they would actually be used.
		...
		However this is a computationally hard problem and therefore expensive to run on Ethereum.\footnote{Nok en ref på at det er svært}
			...
			While vulnerabilities from including the option to change workflows dynamically depends on the implementation, and the feature does not introduce vulnerabilities itself, care must still be taken to ensure everyone involved in the workflow understands the implications of allowing dynamic changes.
			...
			If dynamic changes should be allowed, then which changes should be allowed, and who should be allowed to perform them, is subject to interpretation, and is based on the use case of each single workflow.
		...
		An implementation of this in the mono-contract solution would be fairly simple, as well as relatively cheap if the external workflow is stored in the same contract.
			...
			It is not possible for contracts to access previous states on the Ethereum blockchain, so the timestamps for the executions are not accessible, nor are the transaction hashes, so an approach similar to that in \cite{bachelor} is not possible.
