...
\normalem
	...
	\section{Ethereum}
		...
		Multiple blocks can be published simultaneously, and therefore reference the same preceding block. This situation is called a fork and is normally solved when one of the blocks has more subsequent blocks than the other(s), i.e. the chain with the appropriate block is the longest.


		The specification of Ethereum's blockchain\cite{yellow-paper, ethereum-white-paper} contain some interesting features, which makes it different from most other blockchain technologies.
		...
		Ethereum supports data storage and code execution, which has left the structure of it's blockchain quite different from Bitcoin's.
		Firstly, Ethereum tracks the balance of each account as an unsigned integer, while Bitcoin accounts do not have an explicit balance.
		Instead, Bitcoin uses unspend transactions, \emph{UTXO}'s, which is the transactions that have been sent to a specific account, but not yet been sent from that account. 
		This enables Bitcoin to track the origin of a single BTC all the way back to it's creation, when it was first mined.
		The UTXO structure has both pros and cons, but since Ethereum has to support transactions which fails, but still transfers Eth to another account (see \ref{sec:ethereum-virtual-machine}), another structure had to be implemented none-the-less.
		The UTXO-structure solves the problem of transferring the same digital currency token twice, the so-called \emph{double-spending} problem. 
		Ethereum solves this by adding a counter to each account.
		This counter must be incremented and sent together with each transaction, otherwise the transaction is rejected by the miners.
		...
		In Bitcoin, the integrity of accounts and transactions are verified by building a binary so-called \emph{Merkle-tree} of the transactions, and including the root of this tree in the blocks. 
		A Merkle-tree is a tree of hashes (see figure \ref{fig:bitcoin-merkle-tree}), such that the parent of two leaves is the hash of the leaves concatenated. 
		...
		It it also efficient to store in a block, since the root, which is a single hash, provides the integrity of a tree, and thus only the root-hash needs to be stored in the block.
		By using UTXOs as currency tokens and Merkle-trees to verify transactions, Bitcoin sidesteps the issue of providing integrity for accounts; this is implied by the integrity of an account's corresponding transactions. 

		Ethereum, however, needs data- and code integrity as well, and thus cannot sidestep this issue.
		Where, in Bitcoin, a distinct transaction can only be found in a single Merkle-tree, Ethereum needs to provide a data structure that handles duplication between blocks better, since an account needs to be represented in the structure at any point after it's creation. 
		...
		The Merkle-Patricia tree is essentially a Merkle Radix tree, or Merkle trie, using hex representation as prefixes.
		That is, it stores key-value pairs, in this case account addresses to account/contract structures, and stores pairs together according to their lexicographical prefixes.
		...
		Since the accounts \texttt{a711355} and \texttt{a7f9365} does not share a prefix after \texttt{a7} with other accounts, they are stored directly at this depth, thus saving unnecessary deepening of the tree.
		...
		Figure \ref{fig:merkle-patricia-tree} only shows the \emph{Patricia}-part of the tree, i.e. a radix tree.
		The \emph{Merkle}-part comes from the fact that a reference to a node is not really a reference.
		Instead it is a hash of the node.
		This makes verification possible without having the entire tree, as in the Merkle-tree.  
		...
		The clever this about the Merkle-Patricia tree, is the fact that a lot of data can be reused.
		When a change is made to an account, only the hash of nodes in the branch containing the account needs to recomputed.
		Theoretically, in a full tree containing $20$ byte addresses $= 160$ bit addresses $ = 2^{160}$ unique addresses, the maximal depth of 40 means that only 40 nodes' hashes needs to be recomputed when an account is changed, while the rest of the tree can be reused. 
...
for(i = 0; i < relations.length; i=i+2){
	...
	In order to compare the solutions to each other we have developed a test graph, see figure \ref{fig:exampleWorkflow}, on which we will run a specific sequence of executions, some of which should fail and some of which should succeed (See table \ref{app:test-cases}).
	...
	These features would be fairly use-case dependent and since they would have an effect on the cost of running a workflow, they should only be implemented in instances where they would actually be used.
		...
		Implementing an algorithm that could verify that any given change to a workflow does not make the workflow less restrictive and deploying it on Ethereums blockchain would be possible.
		However this is a very hard problem computationally and a user would be hard pressed to pay for it.\footnote{Nok en ref på at det er svært}

			\subsubsection{Vulnerabilities}
			While including the option to change workflows dynamically does not directly add vulnerabilities on its own to the system, care must be taken to ensure everyone involved in the workflow understands the implications of allowing this.

			Firstly adding an activity $A$ with 1000 condition relations to another activity $B$ would greatly increase the cost of running $B$, as an execution of $B$ would result in checking a 1000 times if $A$ was executed or excluded. 
			...
			Solidity supports implementing this kind of voting on Ethereums blockchain.\cite{voting} 
		...
		An implementation of this in the mono-contract solution would be fairly simple, as well as cheap, as all the workflows are potentially stored in a single contract.
			...
			Suppose a relation is added between workflows $A$ and $B$, if it is an incoming relation (See section \ref{sec:implementation-requirements}), it would have significantly different behaviour than from an outgoing relation.
		...
		In the case where multiple companies have their own Mono-contract implementation, possibly due to their diverse needs for specific features, or the case where the Multi-contract implementation is used, this functionality would be closely tied to the functionality of external relations (See section \ref{sec:external-relations}).
			...
			Figure \ref{fig:external-contract-relations} shows how external contract conditionals can be implemented. 
