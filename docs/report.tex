...
for(i = 0; i < relations.length; i=i+2){
	...
	\section{Discussion}
	%intro - blød overgang til viability
	%Viability
	Having made the above different implementation, we can conclude that the Ethereum platform easily supports DCR.
	There was no need for jumping though hoops, to get several working implementations, unlike in the Bitcoin blockchain \cite{bachelor}.
	However, finding the essential optimizations were not very simple.
	...
	For instance, the pattern where the first successful activity execution has an added cost of at least 9000 gas in all the implementations, which is behaviour we have not been able to figure out.
	Notice from appendices \ref{}This is a common behaviour across  

	%choosing implementation
		% Many workflows vs many executions vs extendability(optimization bliver ødelagt)
	%missing tools for easily checking gas price
...
if (callContract && A.included) //External call
	...
	\section{Example workflow transaction hashes Mono-contract with bitvectors and compressed call data}
	\label{app:example-workflow-transaction-hashes-mono-contract-with-bitvectors-and-compressed-call-data}
		\begin{longtable}{| l | p{0.12\textwidth} | p{0.5\textwidth} |}
			\hline 	
			Action 	 						& Gas cost 	& TxHash on Ropsten Revived 													\\\hline
